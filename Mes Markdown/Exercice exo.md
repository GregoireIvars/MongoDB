...
On emploie notre fonction **Find()** dans lequel on viens préciser que on cherche le champs adresse comportant pas de champs numero. Ensuite, on fait une projection de l'identifiant.
```javascript 
...
```Javascript 
db.salles.find({"avis.date": {$lt: new Date("2019-11-01T00:00:00.000+00:00")}}, {"nom": 1, "avis": 1})
```
## Exercice 11
...
```Javascript 
db.salles.updateMany({"_id": ObjectId("63e26684e3fba2fe2f04e88c")},{$set: {"capacite": 60},$unset: {smac: ""}})
```
